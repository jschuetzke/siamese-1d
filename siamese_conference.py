...
import utils
import models.triplet as models
from models.compound_generator import TripletSeq

#%% load data A

x_val = np.load('./train_a/x_val_noise.npy')
x_val = utils.scale_log(x_val)
x_val = utils.scale_min_max(x_val)
y_val = np.load('./train_a/y_val.npy')
...
for i in range(4):
    ...
    model = models.triplet_phaseid(model_type='triplet_vgg16', optimizer=opt,
                                   flatten=True)
    ...
    model.fit(TripletSeq('./train_a', batch_size, add_noise=True, 
                         augmentation=True, shuffle_before=True),
